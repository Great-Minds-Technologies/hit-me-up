...
function Product() {
  ...
  const fetchUserRole = async () => {
    try {
      const res = await axios.get(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/users/${email}`);
  ...
  const fetchProducts = async () => {
    try {
      setIsLoaded(false);
      const res = await axios.get(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/products/`);
      let pending = [];
  ...
  const approveProduct = async() => {
    ...
    try {
      const res = await axios.put(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/products/updateStatus/${pendingProducts[0]._id}`, {
        status
      });
      fetchProducts();
    } catch (error) {
  ...
  const denyProduct = async() => {
    try {
      const res = await axios.delete(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/products/delete/${pendingProducts[0]._id}`);
      fetchProducts();
    } catch (error) {
      console.log("Error updating status to approved", error);
