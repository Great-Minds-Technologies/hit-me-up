...
function Cart() {
  ...
  const handleRemove = async (id) => {
    try {
      await axios.put(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/users/removeFromCart/${user.email}`,{
        productID: id
      });
      fetchProducts();
    } catch (error) {
   ...
   const fetchProducts = async () => {
      try {
        const userRes = await axios.get(
          `Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/users/${user.email}`
        );
        ...
        const productPromises = productIDs.map(async (id) => {
          try {
            const res = await axios.get(
              `Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/products/${id}`
            );
            return res.data;
          } catch (err) {
  ...
  useEffect(() => {
    async function GetCurrentUser() {
        try {
            const _res = await axios.get("Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/users/logged", {
 ...
 const handleIncrease = async (productId) => {
  ...
  try {
   await axios.put(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/users/increaseFromCart/${user.email}`, {
      productID: productId
    });
    fetchProducts(); 
  } catch (error) {
...
const handleDecrease = async (productId) => {
  ...
  try {
    await axios.put(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/users/decreaseFromCart/${user.email}`, {
      productID: productId
    });
    fetchProducts();
  } catch (error) {
...
const clearCart = async() => {
  ...
  try {
    await axios.put(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/users/clearCart/${user.email}`)
    fetchProducts();
  } catch (error) {
