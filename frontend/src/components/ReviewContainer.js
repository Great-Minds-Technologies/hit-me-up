...
const ReviewContainer = ({productId}) => {
    ...
    async function CaptureReviewInformation () {
        if (itemRating) {
            try {
                ...
                const _tempResult = await axios.post(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/products/${productId}/review/post`, {
                    _rating: parseFloat(itemRating),
                    _productReview: reviewText,
                    _user: user.email
                });
                const _rating = await axios.put(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/products/${productId}/rating`);
            } catch (error) {
                console.log(error);
            }
    ...
    async function GrabReviews() {
        try {
            const _reviews = await axios.get(`Presentation-backend-env.eba-sbkufczm.eu-north-1.elasticbeanstalk.com/api/products/${productId}/reviews`);
